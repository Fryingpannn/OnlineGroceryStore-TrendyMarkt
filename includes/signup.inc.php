...
if(isset($_POST["signup-submit"])){ //checking if came here from click submit
    ...
    if(empty($username) || empty($email) || empty($password)){
        header("Location: ../p6-signup.html?error=emptyfields&first_name=".$username."&mail=".$email); //if empty field, sends error back in header
        exit(); //stops script if error found
    }
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9]*$/", $username)){
        header("Location: ../p6-signup.html?error=invalidmailname"); //checks if email and name are valid
        exit();
    }
    else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: ../p6-signup.html?error=invalidmail&first_name=".$username); //checks if email valid
        exit();
    }
    else if(!preg_match("/^[a-zA-Z0-9]*$/", $username)){
        header("Location: ../p6-signup.html?error=name&email=".$email); //checks if name valid
        exit();
    }
    else if($password !== $passwordRepeat){
        header("Location: ../p6-signup.html?error=passwordcheck&first_name=".$username."&mail=".$email); //checks if password valid
        exit();
    }
        ...
        if(!mysqli_stmt_prepare($stmt, $sql)){
            header("Location: ../p6-signup.html?error=sqlerror"); //checks if statement prepare failed
            exit();
        }
            ...
            if($resultCheck > 0) {
                header("Location: ../p6-signup.html?error=usertaken&email=".$email); //checks if duplicate email
                exit();
            }
                ...
                if(!mysqli_stmt_prepare($stmt, $sql)){
                    header("Location: ../p6-signup.html?error=sqlerror2"); //checks if statement prepare failed
                    exit();
                }
                else{
                    ...
                    header("Location: ../p6-signup.html?signup=success");
                    exit();
                }
            }
        }
    ...
    header("Location: ../p6-signup.html"); //if user didnt come from 'submit', return to sign up
    exit();
}
